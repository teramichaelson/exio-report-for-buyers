...
Either drag all nine files into a new repo through the web interface, or:
...
Do not put anything in a subfolder. Railway needs `package.json` in the repo
root to detect Node, and `server.js` expects the assets beside it.
...
**4. URL is already set**

The canonical tag and the link preview image already point at:

    https://exio-report-for-buyers-production.up.railway.app

Nothing to change unless the domain moves. If it does, update all four
occurrences in `index.html`:

```bash
sed -i '' 's|exio-report-for-buyers-production.up.railway.app|your-new-domain.com|g' index.html
```

Get those wrong and the preview card breaks when someone pastes the link into
Slack or forwards the email.

**5. Sequence link**

Email 2 of the buyer sequence already carries this URL.
...
Everything sits in the repo root, with no subfolders, so the files can be
uploaded one at a time through the GitHub web interface.

Because `server.js` and `package.json` also live in the root, the server only
serves an explicit allowlist: `index.html`, `og.png`, and the two favicons.
Anything else returns 404. If you add an asset later, add its filename to
`ALLOW` in `server.js` or it will not be served.
...
- [ ] Confirm the live domain matches the canonical and og:image tags
